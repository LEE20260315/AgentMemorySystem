...
本轮主题：**效能基准落地 + 融合事务批处理（TODO P2-11）**。新增
`tools/benchmark.py` 把规模拉到十万级拿到第一份量化基线；据此定位并修掉
融合阶段的逐条 fsync —— 首轮 5000 条从 84s 降到 1.8s（45 倍）。

### Added（基准工具）

- 新增 `tools/benchmark.py`：写入 / 查询 / 融合三档基准，临时库跑完即删，
  不碰真实数据根；`--quick` 冒烟档、`--n/--merge-n/--samples` 规模可调、
  `--json` 存档、`--compare` 与历史基线对比（耗时涨跌表）
- 基线与优化后数据存档于 `docs/benchmark/`：
  `baseline-2026-09-07.json`（优化前 v2.5.4）、`v2.5.5.json`（优化后），
  优化前后对比可复现（同 `--seed 20260907`）
- 10 万条写入 4.8~5.7s（约 2 万条/s）；查询全部亚秒；融合首轮
  83.96s → 1.85s、稳态 28.10s → 0.38s（详见 CHANGELOG 下方表格）

### Changed（融合事务批处理）

- `MemoryDatabase` 新增 `batch_transaction()` 上下文与统一提交口 `_commit()`：
  同步循环里的写合并进**一个事务**。与逐条提交唯一区别是不逐条 fsync ——
  循环内后续读取（`_find_similar_in_shared` 的 ID 精确匹配）看到的仍是已写入
  数据，**判定逻辑完全不变**；异常时整轮回滚，反而更安全
- `sync_agent_to_shared` / `sync_shared_to_agent` 两处循环已包进事务

### 根因（为什么融合曾经那么慢）

融合循环每条调 `insert_memory` → `insert_memories_batch([1 条])` →
每条一次 `commit()` = 一次 fsync（本机实测 ≈ 17ms/条）。10 万条写入基准
21k 条/s 是因为分批 1000；逐条提交把融合压到 59 条/s。合并后：

| 场景（10 万条库 / 融合 5000 条） | 基线 v2.5.4 | v2.5.5 | 变化 |
|---|---|---|---|
| 融合 首轮（空共享库） | 83.958s（59.6 条/s） | 1.848s（2705 条/s） | **-97.8%** |
| 融合 稳态（无变化） | 28.095s（178 条/s） | 0.375s（13341 条/s） | **-98.7%** |
| 写入 10 万条 | 4.767s | 5.65s | 运行间波动 |
| get_memory ×200 | 0.767s | 0.851s | 运行间波动 |

写入/查询各场景两次运行差 ±20% 属机器状态波动，量级一致。

### Tests

- 新增 2 条：`test_batch_transaction_semantics`（事务内立即可见 / 落盘 /
  异常回滚 / 回滚后可用 / 嵌套只有最外层生效）、
  `test_merge_uses_single_transaction`（300 条融合亚秒级 + 落盘正确）；
  全量测试 **429/429 全绿**

### 已知瓶颈（下一轮候选）

- `get_memory` 约 4ms/条：主表 + 标签两次查询，可合并为一次 JOIN
- `list_memories` 标签过滤在 10 万库上 0.1s/50 条：`IN (子查询)` + 深分页
  排序，可改 JOIN + 覆盖索引

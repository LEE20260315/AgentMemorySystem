...
- 版本 v2.5.5，遠端 main 與本地同步；測試套件 333 條，**實測 429/429 全綠**
  （2026-09-07 新增事務批處理回歸 2 條 + 標籤斷鏈 4 條 + 插件架構 5 條）
...
### 11. ✅ 效能基準（v2.5.5，2026-09-07 完成）
- **已完成**：新增 `tools/benchmark.py`（寫入 / 查詢 / 融合三檔，`--quick` 冒煙、
  `--json` 存檔、`--compare` 對比，seed 固定可複現），10 萬條基線與優化後
  數據存檔於 `docs/benchmark/`。**據基準定位並修掉融合逐條 fsync**：
  `batch_transaction()` 把同步循環合併為單事務（判定邏輯不變，異常整體回滾），
  融合首輪 83.96s → 1.85s（-97.8%）、穩態 28.10s → 0.38s（-98.7%）。
  回歸測試 2 條，全量 **429/429 全綠**
- **已知瓶頸（下一輪候選）**：`get_memory` 主表+標籤兩次查詢可合併；
  標籤過濾的 `IN (子查詢)` + 深分頁可改 JOIN + 覆蓋索引
...
| 版本 | 日期 | 一句話摘要 |
|------|------|-----------|
| v2.5.5 | 2026-09-07 | 效能基準 + 融合事務批處理：tools/benchmark.py 落地，融合首輪 84s→1.8s（-97.8%）、穩態 28s→0.4s（TODO P2-11） |
...
*最後更新：2026-09-07（v2.5.5：P0/P1 全清 + P2-8/11 完成；剩餘 P2-9/10/12）*

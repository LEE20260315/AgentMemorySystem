...
|---------|------|-----------|
| **v2.4.0** | 2026-08 | **Sync report fidelity** (no new features — fixes the gap between what the system *reports* and what it *does*): the steady-state report falsely claimed "55 new shared memories" every run while the real number was **0** — ① counting now accrues only phase-1 (`agent -> shared`) `inserted`; the phase-2 backflow no longer counts as new; ② `_resolve_conflict` gained a short-circuit: *normalized-content-equal means no change*, killing the per-run replace churn (delete+insert of the same row, net effect zero); ③ merge dedup gained a **normalized-content fallback** (the same memory registered under different ids on each side is no longer missed); ④ **removed the write side-effect in `get_memory()`** (new `track_access` flag; merge comparisons no longer bump `access_count`, which had been inflated to 755 and growing +6 per run — now frozen); fixed **duplicate agent registration** (`.trae-cn\memory` and its parent `.trae-cn` were treated as two agents; new `_is_path_related()`); fixed **`--dry-run` not protecting the merge phase** (extraction and merge used to write for real). Report now splits "new / updated" and spells out when nothing new was added. Verified by replaying three rounds on a DB copy: synced went from 55 to **0**, with row count and id set unchanged. Tests 303/305 (2 pre-existing failures) |
...
├── tombstones.py             # Tombstone mechanism (prevents cross-device resurrection, v2.3.0)
├── safe_io.py                # Safe I/O and data directory resolution
├── memory_sync_app.py        # GUI + system tray + CLI
├── memory_cli.py             # CLI entry point
├── watchdog.py               # Watchdog (auto-restart on crash)
├── setup_agent.py            # Agent initialization script
...
├── docs/                     # Documentation (USAGE_EXAMPLE.md + screenshots; historical docs in docs/archive/)
├── tools/                    # Utilities (shrink_memory_files, repair_fts, log_retention, unify_data_root, ...)
├── AgentMemory/              # Unified data root (agent_*/shared.db/sync state, generated at runtime)
├── TODO.md                   # Roadmap (single source of truth)

...
> **[已归档 2026-09-04]** 本报告为 v2.3.0 时期的同步核查记录，属历史文档。处置结果：
> 问题 1（虚假「55 条新增」）、问题 2（trae 重复登记）、问题 4（dry-run 漏网）、
> 问题 5（merge 死代码）已在 v2.4.0 / v2.4.1 修复或清理（详见 [CHANGELOG.md](../../CHANGELOG.md)）；
> 问题 6 已补齐 `embedding_service` 形参（调用方默认仍不启用，属可选能力）；
> 问题 3（截断不提示丢弃条数）尚未完全关闭，跟踪于 [TODO.md](../../TODO.md)。
